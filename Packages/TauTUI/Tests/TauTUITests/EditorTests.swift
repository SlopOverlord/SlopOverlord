...
struct EditorTests {
    ...
    @Test
    func commandShiftArrowsSelectToLineAndDocumentBoundaries() throws {
        let editor = Editor()
        editor.setText("one two\nthree four")

        editor.handle(input: .key(.arrowLeft, modifiers: [.command, .shift]))
        #expect(editor.getSelection() == TextSelection(
            anchor: CursorPosition(line: 1, col: 10),
            focus: CursorPosition(line: 1, col: 0)))

        editor.handle(input: .key(.escape))
        #expect(editor.getSelection() == nil)

        editor.handle(input: .key(.arrowUp, modifiers: [.command, .shift]))
        #expect(editor.getSelection() == TextSelection(
            anchor: CursorPosition(line: 1, col: 0),
            focus: CursorPosition(line: 0, col: 0)))

        editor.handle(input: .key(.character("X")))
        #expect(editor.getText() == "Xthree four")
    }

    @Test
    func shiftArrowSelectionIsRenderedAndReplacedByTyping() throws {
        let editor = Editor()
        editor.setText("hello")

        editor.handle(input: .key(.arrowLeft, modifiers: [.shift]))
        editor.handle(input: .key(.arrowLeft, modifiers: [.shift]))

        let content = editor.render(width: 20)[1]
        #expect(content.contains("\u{001B}[7ml\u{001B}[0m"))
        #expect(content.contains("\u{001B}[7mo\u{001B}[0m"))

        editor.handle(input: .key(.character("!")))
        #expect(editor.getText() == "hel!")
        #expect(editor.getSelection() == nil)
    }
}
