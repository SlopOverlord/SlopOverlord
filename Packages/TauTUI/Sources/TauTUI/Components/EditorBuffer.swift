...
public struct EditorBuffer: Sendable {
    ...
    public mutating func deleteRange(start: CursorPosition, end: CursorPosition) {
        let startLine = min(max(start.line, 0), self.lines.count - 1)
        let endLine = min(max(end.line, 0), self.lines.count - 1)
        guard startLine < endLine || (startLine == endLine && start.col < end.col) else {
            self.cursorLine = startLine
            self.cursorCol = min(max(start.col, 0), self.lines[startLine].count)
            return
        }

        let safeStartCol = min(max(start.col, 0), self.lines[startLine].count)
        let safeEndCol = min(max(end.col, 0), self.lines[endLine].count)

        if startLine == endLine {
            var line = self.lines[startLine]
            let lower = line.index(line.startIndex, offsetBy: safeStartCol)
            let upper = line.index(line.startIndex, offsetBy: safeEndCol)
            line.removeSubrange(lower..<upper)
            self.lines[startLine] = line
        } else {
            let firstLine = self.lines[startLine]
            let lastLine = self.lines[endLine]
            let firstPrefixEnd = firstLine.index(firstLine.startIndex, offsetBy: safeStartCol)
            let lastSuffixStart = lastLine.index(lastLine.startIndex, offsetBy: safeEndCol)
            let merged = String(firstLine[..<firstPrefixEnd]) + String(lastLine[lastSuffixStart...])
            self.lines.replaceSubrange(startLine...endLine, with: [merged])
        }

        self.cursorLine = startLine
        self.cursorCol = safeStartCol
